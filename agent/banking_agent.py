...
def _extract_text(messages: list[BaseMessage]) -> str:
    ...
    Also strips any <function=...>...</function> blocks that some Llama
    models emit inline as text alongside their proper API tool calls.
    """
    for msg in reversed(messages):
        if isinstance(msg, AIMessage) and msg.content:
            text = str(msg.content)
            # Remove leaked function-call syntax (Llama quirk)
                ...
                while True:
                    ...
                    try:
                        ...
                        if "429" in exc_str or "Too Many Requests" in exc_str:
                            # Pop so the turn can be retried by the user
                            history.pop()
                            print(
                            ...
                            exit_session = True
                        elif "tool_use_failed" in exc_str or (
                            "400" in exc_str and "tool call" in exc_str.lower()
                        ):
                            # The model leaked <function=...> text; Groq rejected it.
                            # Retry silently — the model usually recovers.
                            logger.warning(
                                "tool_use_failed — retrying silently: %s", exc_str[:120]
                            )
                            try:
                                await asyncio.sleep(1)
                                result = await agent.ainvoke({"messages": history})
                                ...
                                print(
                                    "\nAgent: I'm sorry, I ran into a technical issue. "
